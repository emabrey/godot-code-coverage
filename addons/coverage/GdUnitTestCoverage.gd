...
## will result.
static func setup(tree : SceneTree, exclude_paths = DEFAULT_EXCLUDE_PATHS):
...
static func set_targets(per_file_target := DEFAULT_FILE_TARGET, overall_target := DEFAULT_COVERAGE_TARGET):
...
static func finish(verbosity := TestCoverage.Verbosity.FILENAMES) -> bool:
	assert(instance != null, "Before using this method ensure you setup coverage with setup function")
